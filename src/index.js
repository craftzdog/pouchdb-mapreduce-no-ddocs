...
function getDocsToPersist(docId, view, docIdsToChangesAndEmits) {
  ...
  function getKeyValueDocs(metaDoc) {
    ...
    const len = metaDoc.keys.length
    for (let i = 0; i < len; i += CHUNK) {
      const keys = metaDoc.keys.slice(i, i + CHUNK)
      ops = ops.then(function (rows) {
        return view.db.allDocs({
          keys: keys,
          include_docs: true
        }).then(function (res) {
          return rows.concat(res.rows)
        })
      })
    }
    return ops.then(function (rows) {
      return { rows: rows }
    })
  }
  ...
  if (typeof opts.keys !== 'undefined') {
    ...
    var fetchPromises = keys.map(function (key) {
      var viewOpts = {
        ...
        endkey   : toIndexableString([key + (opts.prefix ? '\uffff' : ''), {}])
      };
